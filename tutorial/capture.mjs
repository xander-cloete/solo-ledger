  ...
  return new Promise((resolve, reject) => {
    ...
    open.onsuccess = () => {
      ...
      const stores = [
        ...
        'expenseItems', 'portfolios', 'portfolioBalances',
        'liabilities', 'liabilityBalances', 'liabilityRates',
        'transactions', 'monthState',
      ]
      ...
      // assumedAnnualRate + monthlyContribution drive the Phase 11 projections.
      // We set explicit (modest) rates so the demo projection reads believably —
      // a real portfolio with only ~5 months of history would annualise wildly.
      ...
      // A small, realistic set of debts so the Liabilities page looks lived-in
      // and the dashboard shows its "Debt" panel — sized to keep net worth
      // positive. Two rate rows on the card show a repo-rate change in history.
      ...
      put('liabilityRates', { id: 'lr1', liabilityId: 'l1', effectiveDate: '2026-03-01', annualRate: 20.0 })
      put('liabilityRates', { id: 'lr2', liabilityId: 'l2', effectiveDate: '2025-03-01', annualRate: 11.5 })
...
// One extra, focused shot for the guide: a single portfolio card with its
// "Project" panel open (the projection feature isn't visible until you click in).
console.log('· capturing the projection panel…')
...
const cardHandle = await page.evaluateHandle(() => {
  ...
  return cards.find((c) => c.textContent.includes('Save assumptions')) || cards[0]
})
const cardEl = cardHandle.asElement()
if (cardEl) {
  await cardEl.screenshot({ path: join(SHOTS, 'projection.png') })
  console.log('· captured projection.png')
}
